...
class SubscriptionController extends BaseController
{
    ...
    public function mpesaPay(Request $request)
    {
        ...
        //MAKE PHONE TO HAVE 254 IF NOT ADD AND IF IT HAS LEAVE IT
        ...
        $business_id = request()->session()->get('user.business_id');
        $business_name = request()->session()->get('business.name');
        $description = 'Subscription Payment for ' . $business_name;

        $stkPush = new StkPush();
        $response = $stkPush->initiate($phone_number, $amount, $account_number, $description);
        $data = json_decode($response);
        if (isset($data->ResponseCode) && $data->ResponseCode == '0') {
            //CHACKE IF THE IS TABLE NAMED MPESA PAYMENT IF NOT CREATE AND ADD THE INTITED REANSACTION DETAILS
            $checkTable = DB::select("SHOW TABLES LIKE 'mpesa_payments'");
            if (empty($checkTable)) {
                DB::statement("CREATE TABLE mpesa_payments (
            ...
            $output = ['success' => 0, 'msg' => $data->errorMessage];
            return redirect()
    ...
    public function mpesaVerifyPayment(Request $request)
    {
        $checkout_request_id = $request->input('checkoutRequestID');
        //CHECK IF ITS EMPTY
        if (empty($checkout_request_id)) {
            return response()->json(['status' => 'failed', 'message' => 'CheckoutRequestID is required']);
        }
        $StkPush = new StkPush();
        $response = $StkPush->query($checkout_request_id);
        $responseData = json_decode($response);
        $statusMessages = [
            '1037' => 'Timeout in completing transaction',
            '1032' => 'Transaction has been cancelled by user',
            '1'    => 'The balance is insufficient for the transaction',
            '0'    => 'Transaction was successful'
        ];
        $ResultCode = $responseData->ResultCode ?? null;
        if ($ResultCode == "0") {
            //GET FROM mpesa_payments TABLE
            $mpesa_payment = DB::table('mpesa_payments')->where('CheckoutRequestID', $checkout_request_id)->first();
            $package_id = $mpesa_payment->package_id;
            $business_id = $mpesa_payment->business_id;
            $user_id  = $mpesa_payment->user_id;
            $transactionId = "BMKC" . rand(100000, 999999); 
            //UPDATE THE STATUS OF THE TRANSACTION
            DB::table('mpesa_payments')->where('CheckoutRequestID', $checkout_request_id)->update([
                'TransactionID' => $transactionId,
                'Status' => 'Completed'
            ]);
            $package = Package::active()->find($package_id);
            $this->_add_subscription($business_id, $package, 'mpesa', $transactionId, $user_id);
            return response()->json(['status' => 'success', 'message' => $statusMessages[$ResultCode]]);
        } elseif ($ResultCode == "1032") {
            //UPDATE THE STATUS OF THE TRANSACTION
            DB::table('mpesa_payments')->where('CheckoutRequestID', $checkout_request_id)->update([
                'Status' => 'Cancelled'
            ]);
            return response()->json(['status' => 'failed', 'message' => $statusMessages[$ResultCode]]);
            ...
            return response()->json(['status' => 'pending', 'message' => 'Transaction is still pending']);
        }
    }
